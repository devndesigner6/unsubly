...
interface CTABannerProps { title?: string; description?: string; ctaText?: string; ctaHref?: string }
    ...
    <section className="relative mt-16 overflow-hidden rounded-2xl bg-gradient-to-br from-blue-600 to-indigo-700 px-6 py-12 text-center sm:px-12 sm:py-16">
      <div className="relative mx-auto max-w-2xl">
        ...
        <div className="mt-6 flex flex-col items-center gap-4 sm:flex-row sm:justify-center">
          <Link to={ctaHref} className="inline-flex items-center gap-2 rounded-lg bg-white px-6 py-3 text-sm font-semibold text-blue-700 shadow-lg transition-all hover:bg-blue-50 hover:shadow-xl">
            {ctaText} <RiArrowRightLine className="size-4" />
          </Link>
        </div>
        <div className="mt-6 flex flex-wrap items-center justify-center gap-x-6 gap-y-2 text-sm text-blue-100">
          {["100% Free forever", "No credit card required", "Set up in 2 minutes"].map((item) => (
            <span key={item} className="flex items-center gap-1.5"><RiCheckLine className="size-4 text-blue-200" />{item}</span>
          ))}
        </div>
      </div>
    </section>
